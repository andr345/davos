...
class ResNet(Backbone):
    ...
    def forward(self, x, output_layers=None):
        ...
        if 'image' in output_layers:
            outputs['image'] = x

        if 'layer2hr' in output_layers or 'layer3hr' in output_layers:
            # 2x resolution path
            ...
            x2 = self.layer1(x2)
            x2 = self.layer2(x2)
            if self._add_output_and_check('layer2', x2, outputs, output_layers):
                return outputs
            x2 = self.layer3(x2)
            if self._add_output_and_check('layer3', x2, outputs, output_layers):
                return outputs
