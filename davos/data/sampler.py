...
class LWLSampler(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Label images with all object ids, not just the target
        data['target_ids'] = meta_obj['object_ids']
        if 'labels' in train_anno:
            data['train_labels'] = train_anno['labels']
        if 'labels' in test_anno:
            data['test_labels'] = test_anno['labels']

        if 'distractor_masks' in train_anno:
            data['train_distractor_masks'] = train_anno['distractor_masks']
            have_distractors = [torch.any(m > 0.5) for m in train_anno['distractor_masks']]
            data['have_distractors'] = have_distractors
        if 'distractor_masks' in test_anno:
            data['test_distractor_masks'] = test_anno['distractor_masks']
