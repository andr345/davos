...
def sample_target_adaptive(im, target_bb, search_area_factor, output_sz, mode: str = 'replicate',
                           max_scale_change=None, mask=None, labels=None, distractor_mask=None):
    ...
    if labels is not None:
        lb_crop = labels[y1 + y1_pad:y2 - y2_pad, x1 + x1_pad:x2 - x2_pad]
        lb_crop_padded = F.pad(lb_crop, pad=(x1_pad, x2_pad, y1_pad, y2_pad), mode='constant', value=0)
        labels_out = F.interpolate(lb_crop_padded[None, None].float(), tuple(output_sz.flip(0).long().tolist()), mode='nearest')[0, 0].byte()

    distractor_mask_out = None
    if distractor_mask is not None:
        distractor_mask_crop = distractor_mask[y1 + y1_pad:y2 - y2_pad, x1 + x1_pad:x2 - x2_pad]
        distractor_mask_crop_padded = F.pad(distractor_mask_crop, pad=(x1_pad, x2_pad, y1_pad, y2_pad), mode='constant', value=0)
        distractor_mask_out = F.interpolate(distractor_mask_crop_padded[None, None], tuple(output_sz.flip(0).long().tolist()), mode='nearest')[0, 0]

    crop_box = torch.Tensor([x1, y1, x2 - x1, y2 - y1])

    r = dict(image=im_out, bbox=crop_box)
    if mask is not None:
        r['mask'] = mask_out
    if distractor_mask_out is not None:
        r['distractor_mask'] = distractor_mask_out
...
def target_image_crop(frames, box_extract, box_gt, search_area_factor, output_sz, mode: str = 'replicate',
                      max_scale_change=None, masks=None, labels=None, distractor_masks=None):
    """ For each frame in frames, extracts a square crop centered at box_extract, of area search_area_factor^2
    ...
    args:
        ...
        distractor_masks - Optional distractor masks to apply the same crop.

    returns:
        list - list of image crops
        list - box_gt location in the crop co-ordinates
        list - list of mask crops
        list - list of distractor-mask crops
        """
    ...
    if masks is None:
        for f, a in zip(frames, box_extract):
            r = sample_target_adaptive(f, a, search_area_factor, output_sz, mode, max_scale_change)
            cropped_frames.append(r)
    else:
        for i, (f, a, m) in enumerate(zip(frames, box_extract, masks)):
            dm = distractor_masks[i] if distractor_masks is not None and len(distractor_masks) > 0 else None
            lb = labels[i] if labels is not None and len(labels) > 0 else None
            r = sample_target_adaptive(f, a, search_area_factor, output_sz, mode, max_scale_change, mask=m, distractor_mask=dm, labels=lb)
            cropped_frames.append(r)

    cropped_frames = {k: [d[k] for d in cropped_frames] for k in cropped_frames[0]}

    # find the bb location in the crop
    crop_sz = torch.Tensor(output_sz)
    box_crop = [transform_box_to_crop(bb_gt, crop_bb, crop_sz)
                for bb_gt, crop_bb in zip(box_gt, cropped_frames['bbox'])]
    cropped_frames['bbox'] = box_crop
