...
class Transform:
    ...
    def __init__(self, *transforms):
        ...
        self._valid_inputs = ['image', 'coords', 'bbox', 'mask', 'labels', 'distractor_mask']
        self._valid_args = ['joint', 'new_roll']
        self._valid_all = self._valid_inputs + self._valid_args

    def __call__(self, return_dict=False, **inputs):
        ...
        if not joint_mode:
            out = [self(return_dict=return_dict, **inp) for inp in self._split_inputs(inputs)]
            if return_dict:  # dict of lists
                out = {k: [v[k] for v in out] for k in out[0]}
                return out
            return tuple(list(o) for o in zip(*out))
...
class TransformBase:
    ...
    def __call__(self, **inputs):
        ...
        for var_name, var in input_vars.items():
            if var is not None:
                if var_name.endswith('mask') or var_name == 'labels':
                    transform_func = self.transform_mask
                elif var_name.endswith('image'):
                    transform_func = self.transform_image
                else:
