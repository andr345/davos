...
import davos.data.processing_utils as prutils
...
class LWLProcessing(BaseProcessing):
    ...
    def map_dict(self, in_dict, key_map, inverse=False):
        # Change the names of the keys in in_dict, using the given key map.
        if inverse:
            key_map = {v: k for k, v in key_map.items()}
        return {key_map[k]: v for k, v in in_dict.items() if k in key_map}

    def find_visible_objects(self, labels, as_string=False, vis_threshold=10):
        ret = []  # List of lists of visible objects in the given list of frames.
        for lb in labels:
            object_ids, px_counts = lb.unique(return_counts=True)
            if object_ids[0] == 0:
                object_ids = object_ids[1:]
                px_counts = px_counts[1:]
            visible = object_ids[px_counts > vis_threshold].tolist()
            ret.append(visible)

        ret = str(ret) if as_string else ret
        return ret
    ...
    def __call__(self, data: TensorDict):
        ...
        # same parameters
        if self.transform['joint'] is not None:
            key_map = {'train_images': 'image', 'train_anno': 'bbox', 'train_masks': 'mask', 'train_labels': 'labels', 'train_distractor_masks': 'distractor_mask'}
            r = self.map_dict(data, key_map)
            r = self.transform['joint'](return_dict=True, **r)
            r = self.map_dict(r, key_map, inverse=True)
            data.update(r)

            key_map = {'test_images': 'image', 'test_anno': 'bbox', 'test_masks': 'mask', 'test_labels': 'labels', 'test_distractor_masks': 'distractor_mask'}
            r = self.map_dict(data, key_map)
            r = self.transform['joint'](new_roll=self.new_roll, return_dict=True, **r)
            r = self.map_dict(r, key_map, inverse=True)
            data.update(r)
        ...
        for s in ['train', 'test']:
            ...
            jittered_anno = [self._get_jittered_box(a, s) for a in orig_anno]

            # Extract a crop containing the target
            r = prutils.target_image_crop(
                data[s + '_images'], jittered_anno, orig_anno, self.search_area_factor, self.output_sz, mode=self.crop_type, max_scale_change=self.max_scale_change,
                masks=data[s + '_masks'], distractor_masks=data.get(s + '_distractor_masks', None), labels=data.get(s + '_labels', None))

            # Apply independent transformations to each image
            r = self.transform[s](return_dict=True, joint=False, **r)
            # Count pixels in object labels
            #r['visible_objects'] = self.find_visible_objects(r['labels'], as_string=True, vis_threshold=50)
            key_map = {'image': s + '_images', 'bbox': s + '_anno', 'mask': s + '_masks',
                       'labels': s + '_labels', 'visible_objects': s + '_visible_objects', 'distractor_mask': s + '_distractor_masks'}
            r = self.map_dict(r, key_map)
            data.update(r)
        ...
        for k in data:
            ...
            if k.endswith('_masks') or k.endswith('_labels'):
                data[k] = [v.view(1, *v.shape[-2:]) for v in data[k]]
