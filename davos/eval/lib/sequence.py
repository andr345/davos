...
dv = dict(module="davos.dataset.davis", class_name="DAVIS")
yt = dict(module="davos.dataset.youtubevos", class_name="YouTubeVOS")

dataset_dict = dict(
    dv2016_val=DatasetInfo(**dv, kwargs=dict(version='2016', split='val')),
    dv2017_val=DatasetInfo(**dv, kwargs=dict(version='2017', split='val')),
    dv2017_test_dev=DatasetInfo(**dv, kwargs=dict(version='2017', split='test-dev')),
    dv2017_test_chal=DatasetInfo(**dv, kwargs=dict(version='2017', split='test-challenge')),

    yt2018_test=DatasetInfo(**yt, kwargs=dict(version='2018', split='test')),
    yt2018_valid=DatasetInfo(**yt, kwargs=dict(version='2018', split='valid')),
    yt2018_valid_all=DatasetInfo(**yt, kwargs=dict(version='2018', split='valid', all_frames=True)),

    yt2019_test=DatasetInfo(**yt, kwargs=dict(version='2019', split='test')),
    yt2019_valid=DatasetInfo(**yt, kwargs=dict(version='2019', split='valid')),
    yt2019_valid_all=DatasetInfo(**yt, kwargs=dict(version='2019', split='valid', all_frames=True)),

    yt2018_jjval=DatasetInfo(**yt, kwargs=dict(version='2018', split='jjvalid')),
    yt2019_jjval=DatasetInfo(**yt, kwargs=dict(version='2019', split='jjvalid', cleanup=['starts'])),
    yt2019_jjval_all=DatasetInfo(**yt, kwargs=dict(version='2019', split='jjvalid', all_frames=True, cleanup=['starts'])),
)


def get_dataset_instance(name: str, quiet=False):
    ...
    dataset = getattr(m, dset_info.class_name)(**dset_info.kwargs, quiet=quiet)  # Call the constructor
    return dataset


def load_dataset(name: str, quiet=False):
    return get_dataset_instance(name, quiet).get_sequence_list()


def get_dataset(*args, quiet=False):
    """ Get a single or set of datasets."""
    dset = SequenceList()
    for name in args:
        dset.extend(load_dataset(name, quiet))
    return dset
...
def dataset_sequence_names(name, quiet=False):
    """
    :param name:  Dataset name
    :return: List[Tuple(sequence_name, number_of_frames)]
    """
    sequences = load_dataset(name, quiet=quiet)
    sequences = [(s.name, len(s.frames)) for s in sequences]
    return sequences
