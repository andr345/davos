...
class YouTubeVOS(VOSDatasetBase):
    ...
    def __init__(self, root=None, version='2019', split='train', cleanup=None, all_frames=False, sequences=None,
                 multiobj=True, vis_threshold=10, with_all_labels=False, with_distractors=False, quality="normal", quiet=False):
        """
        args:
        ...
        if all_frames:
            ...
            if quality == "normal":
                ...
                self._jpeg_path = self._jpeg_path.parent / 'JPEGImages_potato'
            else:
        ...
        if meta_path.exists():
            self.gmeta = VOSMeta(filename=meta_path, quiet=quiet)
        else:
