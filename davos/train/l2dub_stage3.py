...
import davos.sys.loading as network_loading
import socket
...
def run(settings):
    settings.description = 'l2 + 32 channel tm + distractors + raft-upsampler, balanced loss'
    ...
    base_sd = network_loading.load_trained_network('davos/l2dub_stage2/LWLNet_ep0080.pth').state_dict()

    print("Setting up upsampler")
    remove = [k for k, v in base_sd.items() if k.startswith("decoder.project")]
    for k in remove:
        del base_sd[k]
    ...
    net = lwl_networks.steepest_descent_resnet50(
        ...
        decoder_input_layers=("layer4", "layer3", "layer2", "layer1", "conv1"),
        upsampler='raft-decoder',
        freeze_vos=True
    )

    r = net.load_state_dict(base_sd, strict=False)
    assert all(k.startswith("decoder.project.") for k in r.missing_keys)
    ...
    # Optimizer
    optimizer = optim.Adam([{'params': actor.net.decoder.parameters(), 'lr': 1e-4}], lr=2e-4)

    lr_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[5, ], gamma=0.2)
    trainer = LTRTrainer(actor, [loader_train, loader_val], optimizer, settings, lr_scheduler)
    trainer.train(6, load_latest=True)
